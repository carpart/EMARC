...
class dbf_class {
    ...
    function dbf_class($filename) {
        ...
        if (strcasecmp($tail, '.dbf')!=0 && strcasecmp($tail, '.DBF')!=0) {
            echo 'Not a valid DBF file !!!'; exit;
        }
        ...
        echo "ord: file: raw: filesize: ".ord($this->_raw[$filesize])."<br>";
        if(!(ord($this->_raw[0]) == 3))  {
			echo "ord: this: raw: 0: ".ord($this->_raw[0])."<br>";
        }
        if(!(ord($this->_raw[0]) == 131))  {
			echo "ord: this: raw: 0: ".ord($this->_raw[0])."<br>";
        }
        if(ord($this->_raw[$filesize]) != 26){
        	echo "ord: this: raw: filesize: ".ord($this->_raw[$filesize])."<br>";
        }
